...
- 命中白名单：`generateContent`/`streamGenerateContent` 返回固定短文本
  （见 usage 章节的精确值），不调用上游；**若请求带 inlineData 输入图 → 400**
  （复审 #6：测活文本模型不接受图片，其 img 单价未定义，直接拒绝更稳）；
...
`client.generate(out_path=...)` 传入落盘路径时，内部走 `_download_to_file` 写文件并
**返回 `image_bytes=None`**（见 `adobe_client.py:1616`）；不传则下载进内存返回 bytes。
本入口两者都要（磁盘统计 + base64），故传 out_path 落盘后**从文件读回 bytes** 做 base64：
...
重新上传（与 chat 路径 `generation.py` 同构）。落盘、`on_generated_file_written`、
失败删残留都与 chat 路径一致。

### 总超时约束（复审 #1、#5）

sub2api 默认 `gateway.response_header_timeout = 600s`；而 adobe2api 底层 HTTP 调用
各有硬编码固定超时（upload/submit/poll 各 60s、download 30s，`adobe_client.py`），
`generate` 的计时还在 submit 完成后才起算，仅把 remaining 传给 `generate(timeout=)`
**无法严格封顶**——最坏可超 600s，sub2api 先断连而 Adobe 任务仍在跑。
...
2. 生成**失败** → 走错误码适配层返回真正的 Google 错误（400/404/429/503/500，
   HTTP 状态正确，非 SSE）；
3. 生成**成功** → 才返回 FastAPI `StreamingResponse(gen(), status_code=200,
   media_type="text/event-stream")`（复审 #7，非 Gin 的 `c.Status`），生成器
   yield 单个 `data: <完整响应 JSON>\n\n` 后结束。Gemini 原生 SSE 无 `[DONE]`，
   流结束即完成。失败分支返回普通 `JSONResponse`（Google 错误），不进 StreamingResponse。
...
- `text_out` 按档位区间随机，锚定真样本 2K=411，按分辨率缩放：
  1K∈[250,320]、2K∈[380,440]、4K∈[520,600]。**注：仅 2K 有实测样本，1K/4K
  为推算区间，待取得真实样本后重新标定**（不作为已验证事实）；
...
**罐头文本模型（测活白名单）的响应与 usage（全确定值，复审 #4）**：

- 固定响应文本常量 `CANNED_TEXT = "ok"`（candidates[0].content.parts[0].text）；
- `candidatesTokenCount = _count_text_tokens("ok") = 1`（由常量算出，非拍脑袋）；
- `promptTokenCount = text_in`（对测活提示词估算，同上包装：空串→0）；
- `thoughtsTokenCount` 不输出、无 candidatesTokensDetails/IMAGE 明细、`serviceTier="standard"`；
- `totalTokenCount = promptTokenCount + 1`。
- `finishReason:"STOP"`、`modelVersion=请求模型名`。
